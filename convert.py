...
def load_from_checkpoint(save_dir, args, vocab, dataset, val_dataset):
    ...
    trainer = Trainer(model, dataset, args, val_dataset, init_epoch=params["epoch"], vocab_size=vocab_size, last_epoch=1)
...
def main():
    ...
    parser.add_argument("--checkpoint_dir", type=str, default="", help="directory to save model")
    ...
    if args.checkpoint_dir == "":
        print("checkpoint directory does not exist... (Please specifie --checkpoint_dir)")
    ...
    dataset = create_dataset(batch_size=1, vocab=vocab, type="train")
    val_dataset = create_dataset(batch_size=1, vocab=vocab, type="validate")

    print("sucessfully loaded dataset...")
    trainer, model = load_from_checkpoint(args.checkpoint_dir, args, vocab, dataset, val_dataset)
    
    # convert model to tflite
    print("converting model to tflite...")
    model.save("img2latex_model_savedmodel", save_format="tf")

    # Convert the SavedModel to TensorFlow.js format
    tfjs.converters.save_keras_model(model, "img2latex_model_tfjs")
